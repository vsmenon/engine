...
class TextDimensions {
  ...
  void updateText(EngineParagraph from, ParagraphGeometricStyle? style) {
    ...
    if (plainText != null) {
      ...
      final html.Element copy = from._paragraphElement.clone(true) as Element;
  ...
  double get height => _readAndCacheMetrics().height as double;
}
...
class ParagraphRuler {
  ...
  int hitTest(ui.ParagraphConstraints constraints, ui.Offset offset) {
    ...
    for (int i = textNodes.length - 1; i >= 0; i--) {
      ...
      // parent element.
      final html.Element parent = node.parentNode as Element;
